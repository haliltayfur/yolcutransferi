      ...
      <div className="bg-white text-black rounded-2xl shadow-2xl p-8 max-w-lg w-full relative animate-fade-in">
        ...
        <div className="mb-2">
          ...
          {summaryData?.fiyatlar?.avg > 0
            ? (
              <div>
                <div className="flex justify-between"><span>Piyasa Ortalaması:</span><span>{summaryData?.fiyatlar.avg}₺</span></div>
                ...
                <div className="flex justify-between text-lg font-bold text-gold mt-2"><span>TOPLAM:</span><span>{summaryData?.fiyatlar.toplam}₺</span></div>
              </div>
            )
            : <div className="text-red-600 text-sm">{summaryData?.message}</div>
          }
        </div>
        {/* Rezervasyon Onay ve Ödeme */}
        <div className="mt-3 flex flex-col items-center gap-3">
          {summaryData?.fiyatlar?.avg > 0 &&
            <button onClick={onPaymentStep} className="bg-gold hover:bg-yellow-400 text-black font-bold rounded-xl px-6 py-3 transition text-lg w-full">
              Rezervasyonu Onayla ve Ödeme Adımına Geç
            </button>
          }
          <button className="underline text-gray-500 hover:text-gray-900 mt-2" onClick={onClose}>İptal / Kapat</button>
        </div>
      </div>
      <style jsx>{`
