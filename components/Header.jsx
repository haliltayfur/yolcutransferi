...
const menuItems = [
  ...
  { name: "Araçlar", href: "/araclar" },"use client";
import { useState, useEffect, useRef } from "react";
import Link from "next/link";
import Image from "next/image";
import { FaWhatsapp, FaInstagram } from "react-icons/fa";
...
export default function Header() {
  ...
  // Hamburger açılır menü pozisyonu
  useEffect(() => {
  ...
  return (
    <header className="w-full bg-black shadow-2xl z-40 relative border-b border-[#FFD70044]">
      <div className="flex items-center justify-between px-7 lg:px-20 py-2 w-full" style={{ minHeight: 100 }}>
        ...
        <nav className="hidden lg:flex items-center gap-1 flex-shrink-0 ml-8">
          ...
          <Link
            href="/login"
            className="header-btn ml-6"
          >
            Giriş Yap
          </Link>
          <Link
            href="/register"
            className="header-btn header-btn-outline ml-3"
          >
            Üye Ol
          </Link>

          {/* Sosyal ikonlar */}
          <a href="https://wa.me/905395267569" target="_blank" rel="noopener noreferrer" className="header-social ml-3">
        ...
        <button
          ref={burgerRef}
          className="lg:hidden flex flex-col items-center justify-center gap-2 p-3 rounded-2xl border-2 border-gold bg-black/95 shadow-xl hover:scale-110 transition-all duration-200"
        ...
        {menuOpen && (
          <nav
            ref={menuBoxRef}
            className="fixed right-4 top-24 z-50 bg-[#161616] border-2 border-gold rounded-2xl shadow-2xl animate-fade-in px-8 py-6"
            style={{
              minWidth: 250,
              maxWidth: 350,
            }}
      ...
      <style jsx>{`
        .header-menu-link {
          color: #eaeaea;
          font-size: 1.13rem; /* Giriş Yap ile aynı */
          font-weight: 700;
          letter-spacing: .02em;
          padding: 14px 26px;
          border-radius: 12px;
        ...
        .header-btn {
          ...
          font-size: 1.13rem;
          border-radius: 17px;
          box-shadow: 0 4px 22px 0 rgba(255,215,0,0.13);
          border: none;
        ...
        .header-btn:hover {
          ...
          color: #101010;
          box-shadow: 0 2px 20px 0 rgba(255,215,0,0.20);
