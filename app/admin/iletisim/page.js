...
export default function AdminIletisim() {
  ...
  const fetchForms = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/iletisim?showRemoved=${showRemoved}&page=${page}&pageSize=${pageSize}`
      );
      if (!res.ok) throw new Error("API Hatası");
      const json = await res.json();
      setForms(json.items || json.forms || []);
  ...
  const handleToggleRemove = async (id, kaldirildi) => {
    await fetch("/api/admin/iletisim", {
      method: "PATCH",
      ...
      <div className="overflow-x-auto bg-black/80 rounded-2xl border-2 border-[#bfa658]">
        {loading ? (
          ...
          <table className="w-full border-collapse min-w-[800px] text-sm">
            ...
            <tbody>
              {forms.map((form, idx) => (
                <tr key={form._id || idx} className={form.kaldirildi ? "bg-[#331] text-gray-500" : "hover:bg-[#231d10] transition"}>
...
// /app/admin/iletisim/page.jsx SON
