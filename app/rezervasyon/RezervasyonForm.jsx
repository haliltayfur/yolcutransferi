...
export default function RezervasyonForm() {
  ...
  function SummaryPopup({ onClose }) {
    ...
    const segmentVehicles = vehicles.filter(v =>
      (!segment || v.segment === segment) &&
      (!transfer || v.transferTypes.includes(transfer)) &&
      (!people || (people > 0 && people <= v.max))
    );
        ...
        <div className="md:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {filteredVehicles.length > 0 ? filteredVehicles.map((v) => (
              ...
              <div className="text-red-400 col-span-3">Uygun araç bulunamadı.</div>
            )}
          </div>
          <div className="mt-2 text-sm text-[#ffeec2] opacity-90">
            Seçtiğiniz segmentteki ve kapasitedeki araçlardan biri size rezerve edilecektir.
          </div>
        </div>
        <div>
