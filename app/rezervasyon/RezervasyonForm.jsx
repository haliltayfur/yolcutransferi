...
import SigortaPopup from "../../components/SigortaPopup.jsx";
import fetchKvkk from "../../utils/fetchKvkk.js";
...
export default function RezervasyonForm() {
  ...
  // MESAFEYİ OTOMATİK ÇEK (inputlar tamamlandıktan sonra)
  useEffect(() => {
    setKm(""); setMin(""); setDistErr("");
    if (!from || !to || !date || !time) return;
    if (transfer === "Dron Transferi") {
      setKm("50"); setMin("60"); setDistErr(""); // Demo
      return;
    }
    getDistance(from, to, date, time).then(res => {
      setKm(res.km);
  ...
  function getBestVehicleText(people, segment) {
    ...
    if (people >= 9) {
      if (segment === "Prime+") return "Sizin için en iyi araç: Maybach veya Mercedes Sprinter (10+ kişilik).";
      return "Sizin için en iyi araç: Mercedes Vito veya Transporter (9-12 kişilik).";
    }
  ...
  // Adet seçimi rezervasyon özetinde olacak
  function handleExtrasUpdate(newList) { setExtras(newList); }
  ...
  // (Tüm popuplar ve formlar diğer dosyalarda. Bu sadece ana iskelet.)
  ...
  return (
    <section className="w-full max-w-3xl mx-auto rounded-3xl shadow-2xl bg-[#19160a] border border-[#bfa658] px-6 md:px-10 py-12 my-10">
      ...
      <form onSubmit={handleSubmit} autoComplete="on" className="flex flex-col gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
          <div>
            ...
            <input value={from} onChange={e => setFrom(e.target.value)} placeholder="Nereden? İlçe/Mahalle/Havalimanı" className="input w-full bg-[#19160a] text-[#ffeec2] border border-[#bfa658] rounded-xl" />
            {fieldErrors.from && <div className="text-red-400 text-xs mt-1">{fieldErrors.from}</div>}
          </div>
          <div>
            <label className="font-bold text-[#bfa658] mb-1 block">Nereye?</label>
            <input value={to} onChange={e => setTo(e.target.value)} placeholder="Nereye? İlçe/Mahalle/Havalimanı" className="input w-full bg-[#19160a] text-[#ffeec2] border border-[#bfa658] rounded-xl" />
            {fieldErrors.to && <div className="text-red-400 text-xs mt-1">{fieldErrors.to}</div>}
          </div>
        </div>
        {(from && to && date && time) && (
          <div className="mb-3 text-[#ffeec2]">
            {distErr ? <span className="text-red-400 font-bold">Mesafe/süre hesaplanamadı</span> : <>
        ...
        <div className="mb-3">
          ...
          <input className="input w-full bg-[#19160a] text-[#ffeec2] border border-[#bfa658] rounded-xl"
            value={pnr} onChange={e => setPnr(e.target.value)} placeholder="Uçuş rezervasyon kodu (varsa)" />
        </div>
        {/* Not */}
        <div className="mb-3">
          <label className="font-bold text-[#bfa658] mb-1 block">Ek Not</label>
          <textarea className="input w-full bg-[#19160a] text-[#ffeec2] border border-[#bfa658] rounded-xl min-h-[60px]"
            value={note} onChange={e => setNote(e.target.value)} placeholder="Eklemek istediğiniz bir not var mı?" />
        </div>
        {/* Ekstralar */}
        <EkstralarAccordion value={extras} onChange={handleExtrasUpdate} onlyCheck={true} />
        {/* Sigorta kutusu */}
        ...
        <div className="flex items-center mt-3 mb-4">
          ...
          <span>
            <span onClick={() => setShowKvkkPopup(true)} className="cursor-pointer underline text-[#ffeec2] hover:text-[#bfa658]">KVKK, Satış ve Diğer Politikaları</span> okudum, onaylıyorum.
          </span>
          <KvkkPopup open={showKvkkPopup} onClose={() => setShowKvkkPopup(false)} text={kvkkText} />
        </div>
        {/* Hata */}
      ...
      <SummaryPopup open={showSummary} onClose={() => setShowSummary(false)} {...{
        ...
        extras, sigorta, sigortaTutar, transferUcreti, vehicleText, onNext: () => { setShowSummary(false); setShowPayment(true); }
      }} />
      <PaymentPopup open={showPayment} onClose={() => setShowPayment(false)} {...{
        transferUcreti, sigortaTutar, extras, onNext: () => { setShowPayment(false); setShowThanks(true); }
      }} />
      <TesekkurPopup open={showThanks} onClose={() => setShowThanks(false)} {...{ name, email }} />
    </section>
  );
}
