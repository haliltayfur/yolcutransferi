  ...
  return (
    <div className="space-y-4">
      {extrasListByCategory.map((cat, i) => (
        <div
          key={cat.category}
          className="border-b border-[#bfa658] last:border-b-0"
        >
          <button
            type="button"
          ...
          {openIndex === i && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 pb-2">
              {cat.items.map((extra) => (
                <div
                  key={extra.key}
                  className="flex items-center gap-3 bg-[#19160a] border border-[#bfa658] rounded-xl px-4 py-2"
                >
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) => {
                      if (e.target.checked)
                        ...
                        setSelectedExtras(
                          selectedExtras.filter((k) => k !== extra.key)
                        );
                    }}
